...
function buildSystemPrompt(facts: Fact[]): string {
  ...
  return [
    ...
    DRAFTING_VOICE,
    '',
    'ACTIONS ARE TOOL CALLS — THE MOST IMPORTANT RULE: you can only act by',
    'calling a tool in THIS turn. NEVER say something was logged, created,',
    'updated, or done unless the matching tool call ran in this same turn and',
    'returned success — reporting an action you did not perform is lying to',
    'Jayden about his money. If you announce you are about to do something',
    '("logging these now"), you MUST make those tool calls before ending the',
    'turn, one call per record. A plan or promise in an earlier message is',
    'NOT an action: if the tools have not run yet, run them now or say',
